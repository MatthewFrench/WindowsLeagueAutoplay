    ...
    public class ATimer
    {
        ...
        public void Start()
        {
            ...
            else if (_timerType == 3)
            {
                ...
                mTimerId = timeSetEvent((uint)_interval, uint.MaxValue, mHandler, IntPtr.Zero, EVENT_TYPE);
        ...
        [DllImport("winmm.dll")]
        private static extern int timeSetEvent(uint delay, uint resolution,
                                                TimerEventHandler handler, IntPtr user, uint eventType);
        [DllImport("winmm.dll")]
